...
public class IndexDiskIO {
    ...
    public void save_index_to_disk(Map<String, Integer> lexicon,
        ...
        // save lexicon
        try (BufferedWriter writer = Files.newBufferedWriter(Paths.get(outputPath, "lexicon.txt"))) {
        ...
        // save documents
        try (BufferedWriter writer = Files.newBufferedWriter(Paths.get(outputPath, "documents.txt"))) {
        ...
        // save postings
        try (BufferedWriter writer = Files.newBufferedWriter(Paths.get(outputPath, "postings.txt"))) {
    ...
    public IndexData read_index_from_disk(String indexPath) throws IOException {
        ...
        // read lexicon
        try (BufferedReader reader = Files.newBufferedReader(Paths.get(indexPath, "lexicon.txt"))) {
            String line;
        ...
        // read documents
        try (BufferedReader reader = Files.newBufferedReader(Paths.get(indexPath, "documents.txt"))) {
            String line;
            while ((line = reader.readLine()) != null) {
                String[] parts = line.split("\t");
                if (parts.length == 3) {
        ...
        // read postings
        try (BufferedReader reader = Files.newBufferedReader(Paths.get(indexPath, "postings.txt"))) {
            String line;
            while ((line = reader.readLine()) != null) {
                String[] parts = line.split("\t");
                if (parts.length == 2) {
